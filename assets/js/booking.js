(() => {
    ...
    const dateInput = modal?.querySelector(
        "[data-booking-datepicker]",
    );
    const monthLabel = modal?.querySelector(
        "[data-booking-month-label]",
    );
    const monthPrev = modal?.querySelector(
        "[data-booking-month-prev]",
    );
    const monthNext = modal?.querySelector(
        "[data-booking-month-next]",
    );
    const timesElement = modal?.querySelector(
        "[data-booking-times]",
    );
    const datesLoader = modal?.querySelector(
        "[data-booking-dates-loader]",
    );
    const peopleMinus = modal?.querySelector(
        "[data-booking-people-minus]",
    );
    const peoplePlus = modal?.querySelector(
        "[data-booking-people-plus]",
    );
    const peopleOutput = modal?.querySelector(
        "[data-booking-people-output]",
    );
    ...
    const infoOpenButton = modal?.querySelector(
        "[data-booking-info-open]",
    );
    const infoPopup = modal?.querySelector(
        "[data-booking-info-popup]",
    );
    const infoDialog = infoPopup?.querySelector(
        ".booking-info-popup__dialog",
    );
    ...
    const priceTotal = modal?.querySelector(
        "[data-booking-price-total]",
    );
    const priceSidebarRow = modal?.querySelector(
        "[data-booking-price-sidebar-row]",
    );
    const priceSidebar = modal?.querySelector(
        "[data-booking-price-sidebar]",
    );
    const priceDetails = modal?.querySelector(
        "[data-booking-price-details]",
    );
    ...
    const getVisibleMonthIndex = () => {
        ...
        return (
            datepicker.currentYear * 12 + datepicker.currentMonth
        );
    };
    ...
    const fetchAvailableDates = async ({ year, month }) => {
        ...
        const data = await fetch(
            `/api/booking/dates?${params.toString()}`,
            { cache: "no-store" },
    ...
    const fetchAvailableTimes = async ({ date }) => {
        ...
        const data = await fetch(
            `/api/booking/times?${params.toString()}`,
            { cache: "no-store" },
        ).then(readJsonResponse);
    ...
    const calculateBookingPrice = () => {
        ...
        return (
            firstSetCount * FIRST_SET_PRICE +
            nextSetCount * NEXT_SET_PRICE
        );
    };
    ...
    const hasMinimumSelectedTimes = () =>
        bookingState.selectedTimes.length >=
        bookingState.peopleCount;
    ...
    const getMinimumTimesPlaceholder = () =>
        `Выберите минимум ${formatSlotCount(bookingState.peopleCount).replace(" ", "<br />")} для ${bookingState.peopleCount} чел.`;
    ...
    const updatePriceSummary = () => {
        ...
        const shouldHidePrice =
            isMobileBookingLayout() && !hasSelectedTime;
        ...
        if (priceSidebarRow) {
            priceSidebarRow.hidden = shouldHidePrice;
        }

        if (priceSidebar) {
            priceSidebar.textContent = formatPrice(total, "л");
        }

        if (priceDetails) {
            priceDetails.textContent = setCount
                ? `${bookingState.peopleCount} чел. x ${setCount} сет(ов)`
                : "Выберите время для расчета";
        }
    };
    ...
    const updateSubmitState = () => {
        ...
        const shouldShowPlaceholder = !hasEnoughSelectedTimes;

        submitButton.hidden = shouldShowPlaceholder;
        if (submitPlaceholder) {
            submitPlaceholder.hidden = !shouldShowPlaceholder;
            submitPlaceholder.innerHTML = hasSelectedTime
                ? getMinimumTimesPlaceholder()
        ...
        submitButton.setAttribute(
            "aria-busy",
            String(bookingState.isSubmitting),
        );
    ...
    const updatePeopleOutput = () => {
        if (peopleOutput) {
            peopleOutput.textContent = String(
                bookingState.peopleCount,
            );
        }

        peopleMinus?.toggleAttribute(
            "disabled",
            bookingState.peopleCount <= 1,
        );
        peoplePlus?.toggleAttribute(
            "disabled",
            bookingState.peopleCount >= 8,
        );
    };
    ...
    const renderTimes = () => {
        ...
        if (bookingState.isLoadingTimes) {
            ...
            loader.className =
                "booking-times__empty booking-times__loader";
        ...
        bookingState.availableTimes.forEach(
            ({ time, available }) => {
                ...
                button.addEventListener("click", () => {
                    if (bookingState.selectedTimes.includes(time)) {
                        bookingState.selectedTimes =
                            bookingState.selectedTimes.filter(
                                (selectedTime) =>
                                    selectedTime !== time,
                            );
                    } else {
    ...
    const loadTimesForSelectedDate = async () => {
        ...
        bookingState.selectedTimes =
            bookingState.selectedTimes.filter((time) =>
    ...
    const applyDateAvailability = () => {
        ...
        const firstAvailableDate = Array.from(
            bookingState.availableDates,
        )[0];

        if (
            bookingState.selectedDate &&
            !bookingState.availableDates.has(
                bookingState.selectedDate,
            )
        ) {
            bookingState.selectedDate = "";
    ...
    const setPeopleCount = (nextCount) => {
        bookingState.peopleCount = Math.min(
    ...
    const isInfoPopupOpen = () =>
        Boolean(infoPopup && !infoPopup.hidden);
    ...
    const closeInfoPopup = ({ restoreFocus = true } = {}) => {
        ...
        if (
            restoreFocus &&
            lastFocusedInfoElement instanceof HTMLElement
        ) {
